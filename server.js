...
const IDLE_ROAM_INTERVAL = 8000;
const bounds = { minX:-25, maxX:25, minZ:-44, maxZ:24 };
const players = new Map();
const granny = { x:0, z:-24, state:'ROAM', targetId:null, roamX:0, roamZ:-24, path:[], pathIndex:0, pathTarget:null, pathTimer:0, nextRoamAt:0 };
...
function chooseRoam(){const p=pickTarget();if(p){granny.roamX=p.x;granny.roamZ=p.z;return}granny.roamX=(Math.random()*40)-20;granny.roamZ=(Math.random()*54)-30;granny.nextRoamAt=Date.now()+IDLE_ROAM_INTERVAL}
...
function updateGranny(){const now=Date.now();const current=granny.targetId?players.get(granny.targetId):null;if(current&&distance(granny,current)<=CHASE_RELEASE_DISTANCE)granny.state='CHASE';else{const nearby=[...players.values()].filter(p=>distance(granny,p)<=CHASE_DISTANCE);if(nearby.length){nearby.sort((a,b)=>b.noise-a.noise);granny.targetId=nearby[0].id;granny.state='CHASE';granny.pathTarget=null}else{granny.targetId=null;granny.state='ROAM';if(now>=granny.nextRoamAt||Math.hypot(granny.x-granny.roamX,granny.z-granny.roamZ)<1.5){chooseRoam();granny.pathTarget=null}}}if(granny.state==='CHASE'){const target=players.get(granny.targetId);if(target)moveAlongPath(target.x,target.z,CHASE_SPEED)}else{const target=pickTarget();if(target&&now-target.lastMoveAt<5000){granny.roamX=target.x;granny.roamZ=target.z;granny.nextRoamAt=now+IDLE_ROAM_INTERVAL}moveAlongPath(granny.roamX,granny.roamZ,ROAM_SPEED)}}
