...
function updateGranny(){const current=granny.targetId?players.get(granny.targetId):null;if(current&&distance(granny,current)<=CHASE_RELEASE_DISTANCE)granny.state='CHASE';else{const nearby=[...players.values()].filter(p=>distance(granny,p)<=CHASE_DISTANCE);if(nearby.length){nearby.sort((a,b)=>b.noise-a.noise);granny.targetId=nearby[0].id;granny.state='CHASE'}else{granny.targetId=null;granny.state='ROAM';if(Math.hypot(granny.x-granny.roamX,granny.z-granny.roamZ)<1.5)chooseRoam()}}if(granny.state==='CHASE'){const target=players.get(granny.targetId);if(target)moveToward(granny.x,granny.z,target.x,target.z,CHASE_SPEED)}else{const target=pickTarget();if(target){granny.roamX=target.x;granny.roamZ=target.z}moveToward(granny.x,granny.z,granny.roamX,granny.roamZ,ROAM_SPEED)}}
...
wss.on('connection',ws=>{if(players.size>=MAX_PLAYERS){send(ws,{type:'error',error:'server-full',maxPlayers:MAX_PLAYERS});ws.close();return}const id=randomUUID(),p={id,ws,x:0,z:18,rotation:0,lastMoveAt:Date.now(),speed:0,noise:0};players.set(id,p);send(ws,{type:'welcome',id,players:players.size,maxPlayers:MAX_PLAYERS});broadcast(snapshot());ws.on('message',raw=>{let m;try{m=JSON.parse(raw.toString())}catch{return}if(m.type==='move'){const now=Date.now(),oldX=p.x,oldZ=p.z,x=Number(m.x),z=Number(m.z);if(!Number.isFinite(x)||!Number.isFinite(z))return;validMove(p,x,z);p.rotation=Number.isFinite(m.rotation)?m.rotation:0;const moved=Math.hypot(p.x-oldX,p.z-oldZ);p.speed=moved/(Math.max(1,now-p.lastMoveAt)/1000);if(moved>.02)p.lastMoveAt=now;p.noise=Math.min(1,p.speed/7)}});const cleanup=()=>{if(players.delete(id)){if(granny.targetId===id)granny.targetId=null;broadcast(snapshot())}};ws.on('close',cleanup);ws.on('error',cleanup)});
setInterval(()=>{updateGranny();broadcast(snapshot())},TICK_MS);
server.listen(PORT,'0.0.0.0',()=>console.log(`Granny server listening on ${PORT} — max ${MAX_PLAYERS} players — chase ${CHASE_DISTANCE}`));
